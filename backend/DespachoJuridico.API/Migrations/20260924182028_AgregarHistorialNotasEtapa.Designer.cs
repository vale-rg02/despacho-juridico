// <auto-generated />
using System;
using DespachoJuridico.API.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace DespachoJuridico.API.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260924182028_AgregarHistorialNotasEtapa")]
    partial class AgregarHistorialNotasEtapa
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("DespachoJuridico.API.Models.AcuerdoScrapeado", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("CiudadDestino")
                        .HasColumnType("text");

                    b.Property<string>("Confianza")
                        .HasColumnType("text");

                    b.Property<bool>("DescartadoManualmente")
                        .HasColumnType("boolean");

                    b.Property<bool>("EsExhorto")
                        .HasColumnType("boolean");

                    b.Property<int>("ExpedienteId")
                        .HasColumnType("integer");

                    b.Property<DateOnly>("FechaAcuerdo")
                        .HasColumnType("date");

                    b.Property<DateTime>("FechaDetectado")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("IdUnidad")
                        .HasColumnType("integer");

                    b.Property<string>("NombreJuzgado")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("NotificacionEnviada")
                        .HasColumnType("boolean");

                    b.Property<string>("NumeroExpediente")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("Oculto")
                        .HasColumnType("boolean");

                    b.Property<string>("Partes")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("RecordatorioPendienteEnviado")
                        .HasColumnType("boolean");

                    b.Property<bool>("RegistradoManualmente")
                        .HasColumnType("boolean");

                    b.Property<string>("Sintesis")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("TipoAsunto")
                        .HasColumnType("text");

                    b.Property<bool>("Visto")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.HasIndex("ExpedienteId", "FechaAcuerdo", "Sintesis")
                        .IsUnique();

                    b.ToTable("AcuerdosScrapeados");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Banco", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreadoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Direccion")
                        .HasColumnType("text");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Telefono")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Bancos");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.BitacoraCambio", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Accion")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Detalle")
                        .HasColumnType("text");

                    b.Property<int>("ExpedienteId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("UsuarioId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ExpedienteId");

                    b.HasIndex("UsuarioId");

                    b.ToTable("BitacoraCambios");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Cita", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreadoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Descripcion")
                        .HasColumnType("text");

                    b.Property<int?>("ExpedienteId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("FechaHora")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("ReminderEnviado")
                        .HasColumnType("boolean");

                    b.Property<string>("Titulo")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("UsuarioId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ExpedienteId");

                    b.HasIndex("UsuarioId");

                    b.ToTable("Citas");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.EstadoScraper", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime?>("UltimaCorridaCompletaEn")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.ToTable("EstadosScraper");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.EtapaCatalogo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("EsDiasHabiles")
                        .HasColumnType("boolean");

                    b.Property<int?>("EtapaPadreId")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Orden")
                        .HasColumnType("integer");

                    b.Property<int?>("TerminoDias")
                        .HasColumnType("integer");

                    b.Property<string>("TipoJuicio")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("EtapaPadreId");

                    b.ToTable("EtapasCatalogo");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Expediente", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("AccionPendiente")
                        .HasColumnType("text");

                    b.Property<DateTime>("ActualizadoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int?>("BancoId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreadoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("CreadoPorId")
                        .HasColumnType("integer");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("EtapaActual")
                        .HasColumnType("text");

                    b.Property<int?>("ExpedienteRelacionadoId")
                        .HasColumnType("integer");

                    b.Property<string>("Juzgado")
                        .HasColumnType("text");

                    b.Property<string>("Materia")
                        .HasColumnType("text");

                    b.Property<string>("Notas")
                        .HasColumnType("text");

                    b.Property<string>("NumeroExpediente")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("ParteDemandada")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Prioridad")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Sede")
                        .HasColumnType("text");

                    b.Property<string>("TipoJuicio")
                        .HasColumnType("text");

                    b.Property<int?>("UsuarioAsignadoId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("BancoId");

                    b.HasIndex("CreadoPorId");

                    b.HasIndex("ExpedienteRelacionadoId");

                    b.HasIndex("UsuarioAsignadoId");

                    b.ToTable("Expedientes");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.ExpedienteAcceso", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreadoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("ExpedienteId")
                        .HasColumnType("integer");

                    b.Property<int>("UsuarioId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("UsuarioId");

                    b.HasIndex("ExpedienteId", "UsuarioId")
                        .IsUnique();

                    b.ToTable("ExpedienteAccesos");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.HistorialEtapa", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("Atendido")
                        .HasColumnType("boolean");

                    b.Property<int?>("EtapaCatalogoId")
                        .HasColumnType("integer");

                    b.Property<int>("ExpedienteId")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("FechaCompletada")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("FechaInicio")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("FechaLimite")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("NotasLegado")
                        .HasColumnType("text");

                    b.Property<int>("RegistradoPorId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("EtapaCatalogoId");

                    b.HasIndex("ExpedienteId");

                    b.HasIndex("RegistradoPorId");

                    b.ToTable("HistorialEtapas");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.JuzgadoCatalogo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("SedeId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("SedeId", "Nombre")
                        .IsUnique();

                    b.ToTable("JuzgadosCatalogo");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.NotaEtapa", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreadoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("CreadoPorId")
                        .HasColumnType("integer");

                    b.Property<int>("HistorialEtapaId")
                        .HasColumnType("integer");

                    b.Property<string>("Texto")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("CreadoPorId");

                    b.HasIndex("HistorialEtapaId");

                    b.ToTable("NotasEtapa");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Notificacion", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Canal")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreadoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("DestinatarioEmail")
                        .HasColumnType("text");

                    b.Property<int>("DiasAnticipacion")
                        .HasColumnType("integer");

                    b.Property<bool>("Enviada")
                        .HasColumnType("boolean");

                    b.Property<int>("ExpedienteId")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("FechaEnvio")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int?>("HistorialEtapaId")
                        .HasColumnType("integer");

                    b.Property<string>("Mensaje")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("ExpedienteId");

                    b.HasIndex("HistorialEtapaId");

                    b.ToTable("Notificaciones");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.SedeCatalogo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Nombre")
                        .IsUnique();

                    b.ToTable("SedesCatalogo");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Usuario", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean");

                    b.Property<DateTime>("CreadoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("EsCuentaSoporte")
                        .HasColumnType("boolean");

                    b.Property<int>("NivelAcceso")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Rol")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("ScraperEnProgreso")
                        .HasColumnType("boolean");

                    b.Property<DateTime?>("ScraperIniciadoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("UltimaConsultaScraperEn")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.ToTable("Usuarios");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.AcuerdoScrapeado", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.Expediente", "Expediente")
                        .WithMany()
                        .HasForeignKey("ExpedienteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Expediente");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.BitacoraCambio", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.Expediente", "Expediente")
                        .WithMany("Bitacora")
                        .HasForeignKey("ExpedienteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("DespachoJuridico.API.Models.Usuario", "Usuario")
                        .WithMany("Bitacora")
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Expediente");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Cita", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.Expediente", "Expediente")
                        .WithMany()
                        .HasForeignKey("ExpedienteId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("DespachoJuridico.API.Models.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Expediente");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.EtapaCatalogo", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.EtapaCatalogo", "EtapaPadre")
                        .WithMany("Subetapas")
                        .HasForeignKey("EtapaPadreId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("EtapaPadre");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Expediente", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.Banco", "Banco")
                        .WithMany("Expedientes")
                        .HasForeignKey("BancoId");

                    b.HasOne("DespachoJuridico.API.Models.Usuario", "CreadoPor")
                        .WithMany("ExpedientesCreados")
                        .HasForeignKey("CreadoPorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("DespachoJuridico.API.Models.Expediente", "ExpedienteRelacionado")
                        .WithMany()
                        .HasForeignKey("ExpedienteRelacionadoId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("DespachoJuridico.API.Models.Usuario", "UsuarioAsignado")
                        .WithMany("ExpedientesAsignados")
                        .HasForeignKey("UsuarioAsignadoId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Banco");

                    b.Navigation("CreadoPor");

                    b.Navigation("ExpedienteRelacionado");

                    b.Navigation("UsuarioAsignado");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.ExpedienteAcceso", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.Expediente", "Expediente")
                        .WithMany("Accesos")
                        .HasForeignKey("ExpedienteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("DespachoJuridico.API.Models.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Expediente");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.HistorialEtapa", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.EtapaCatalogo", "EtapaCatalogo")
                        .WithMany("Historial")
                        .HasForeignKey("EtapaCatalogoId");

                    b.HasOne("DespachoJuridico.API.Models.Expediente", "Expediente")
                        .WithMany("Historial")
                        .HasForeignKey("ExpedienteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("DespachoJuridico.API.Models.Usuario", "RegistradoPor")
                        .WithMany("HistorialRegistrado")
                        .HasForeignKey("RegistradoPorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("EtapaCatalogo");

                    b.Navigation("Expediente");

                    b.Navigation("RegistradoPor");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.JuzgadoCatalogo", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.SedeCatalogo", "Sede")
                        .WithMany("Juzgados")
                        .HasForeignKey("SedeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Sede");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.NotaEtapa", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.Usuario", "CreadoPor")
                        .WithMany()
                        .HasForeignKey("CreadoPorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("DespachoJuridico.API.Models.HistorialEtapa", "HistorialEtapa")
                        .WithMany("Notas")
                        .HasForeignKey("HistorialEtapaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CreadoPor");

                    b.Navigation("HistorialEtapa");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Notificacion", b =>
                {
                    b.HasOne("DespachoJuridico.API.Models.Expediente", "Expediente")
                        .WithMany("Notificaciones")
                        .HasForeignKey("ExpedienteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("DespachoJuridico.API.Models.HistorialEtapa", "HistorialEtapa")
                        .WithMany("Notificaciones")
                        .HasForeignKey("HistorialEtapaId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Expediente");

                    b.Navigation("HistorialEtapa");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Banco", b =>
                {
                    b.Navigation("Expedientes");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.EtapaCatalogo", b =>
                {
                    b.Navigation("Historial");

                    b.Navigation("Subetapas");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Expediente", b =>
                {
                    b.Navigation("Accesos");

                    b.Navigation("Bitacora");

                    b.Navigation("Historial");

                    b.Navigation("Notificaciones");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.HistorialEtapa", b =>
                {
                    b.Navigation("Notas");

                    b.Navigation("Notificaciones");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.SedeCatalogo", b =>
                {
                    b.Navigation("Juzgados");
                });

            modelBuilder.Entity("DespachoJuridico.API.Models.Usuario", b =>
                {
                    b.Navigation("Bitacora");

                    b.Navigation("ExpedientesAsignados");

                    b.Navigation("ExpedientesCreados");

                    b.Navigation("HistorialRegistrado");
                });
#pragma warning restore 612, 618
        }
    }
}
